...
def html_template_div_modal_message(request):
    return loader.render_to_string(
        template_name='hpc/_include_/div_modal/message/message.html',
        context={},
        request=request
    )
...
def html_template_div_modal_message_message(request):
    return loader.render_to_string(
        template_name='hpc/_include_/div_modal/_include_/message/message.html',
        context={
...
def jsonresponse_error(request):
    if len(messages.get_messages(request=request)) <= 0:
        messages.add_message(request, messages.ERROR, _('HPC_MODULE_MESSAGE ERROR.'))
    data = dict()
