...
@security_decorators.required_security_user_is_ldapuser_or_importedldapuser(security_from_module=security_utils.SECURITY_FROM_MODULE_HPC)
def index(request):
    context = dict()
    return shortcuts.render(
        request=request,
        context=context,
        template_name='hpc/application.html'
    )
...
def title(request):
    ...
    data['HTML_TITLE'] = utils.html_template(
        request=request,
        context=dict(),
        template='hpc/_include_/title/title.html'
    )
...
def div_load(request):
    ...
    data['HTML_DIV_LOAD'] = utils.html_template(
        request=request,
        context=dict(),
        template='hpc/_include_/div_load/div_load.html'
    )
    data['BOOLEAN_ERROR'] = False
    return http.JsonResponse(data)
...
def div_header(request):
    ...
    data['HTML_DIV_HEADER'] = utils.html_template(
        request=request,
        context=dict(),
        template='hpc/_include_/div_header/div_header.html'
    )
    data['BOOLEAN_ERROR'] = False
    return http.JsonResponse(data)
...
def div_leftside(request):
    ...
    data['HTML_DIV_LEFTSIDE'] = utils.html_template(
        request=request,
        context=dict(),
        template='hpc/_include_/div_leftside/div_leftside.html'
    )
    data['BOOLEAN_ERROR'] = False
    return http.JsonResponse(data)
...
def div_center_div_content(request):
    ...
    data['HTML_DIV_CENTER_DIV_CONTENT'] = utils.html_template(
        request=request,
        context=dict(),
        template='hpc/_include_/div_center/div_content/div_content.html'
    )
    data['BOOLEAN_ERROR'] = False
    return http.JsonResponse(data)
...
def div_center_div_footer(request):
    ...
    data['HTML_DIV_CENTER_DIV_FOOTER'] = utils.html_template(
        request=request,
        context=dict(),
        template='hpc/_include_/div_center/div_footer/div_footer.html'
    )
    data['BOOLEAN_ERROR'] = False
    return http.JsonResponse(data)
...
def login(request):
    return security_utils.jsonresponse_login(
        request=request,
        security_from_module=security_utils.SECURITY_FROM_MODULE_HPC
    )
...
@security_decorators.required_request_is_ajax()
@security_decorators.required_security_user(security_from_module=security_utils.SECURITY_FROM_MODULE_HPC)
def logout(request):
