# -*- coding: utf-8 -*-
from src.security import models, tasks, utils
...
class RequestedLOCALUserDetail(forms.ModelForm):
...
class RequestedLOCALUserApprove(forms.ModelForm):
    class Meta:
        model = models.RequestedLOCALUser
        fields = ['id', ]
    ...
    def approve(self):
        ...
        tasks.task___security_login_request_approve_send_mail.apply_async(
                    args=[],
                    kwargs={
                        'string___user_model': utils.SECURITY_USER_MODEL_LOCALUSER_TEXT,
...
class RequestedLOCALUserDisapprove(forms.ModelForm):
    class Meta:
        model = models.RequestedLOCALUser
        fields = ['id', ]
